...
//   FeedBloc({required this.getPosts, required this.refreshPosts}) : super(const FeedState()) {
...
//     final posts = await getPosts(page: 1, limit: _limit);
//     _page = 1;
//     _hasMore = posts.length == _limit;
//     emit(state.copyWith(status: FeedStatus.success, posts: posts, hasMore: _hasMore));
//   }
...
//     final more = await getPosts(page: nextPage, limit: _limit);
//     _page = nextPage;
//     _hasMore = more.length == _limit;
//     emit(state.copyWith(
//       posts: List.of(state.posts)..addAll(more),
//       hasMore: _hasMore,
//       loadingMore: false,
//     ));
//     _isFetching = false;
...
//     final fresh = await refreshPosts(limit: _limit);
//     _page = 1;
//     _hasMore = fresh.length == _limit;
//     emit(state.copyWith(status: FeedStatus.success, posts: fresh, hasMore: _hasMore));
//   }
// }
...
//     on<FeedLoadMore>(_onLoadMore);
//     on<FeedRefreshed>(_onRefreshed);
//     on<FeedPostPatched>(_onPostPatched); // NEW
//   }
...
//   // NEW: replace a post in the list when we receive an updated entity
